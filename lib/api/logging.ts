...
const SENSITIVE_KEY_PATTERN =
  /(password|secret|token|authorization|cookie|api[-_]?key|card|cvv|expiry|payment|passcode|otp|email|auth|session|credential|private[-_]?key|access[-_]?key|refresh[-_]?token|magic[-_]?link|verification|message|content|prompt|payload|provider|customer|attachment|url|uri|link)/i
...
const SIGNED_URL_PATTERN = /https?:\/\/[^\s]+(?:x-amz-|sig(?:nature)?=|token=|expires=)[^\s]*/i
...
function redactValue(value: unknown): unknown {
  if (typeof value === "string") {
    if (TOKEN_VALUE_PATTERN.test(value) || COOKIE_VALUE_PATTERN.test(value) || EMAIL_VALUE_PATTERN.test(value) || SIGNED_URL_PATTERN.test(value)) {
      return "[REDACTED]"
    }
