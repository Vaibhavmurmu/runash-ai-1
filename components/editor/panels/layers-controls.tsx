...
export default function LayersControls({ timeline, onTimelineChange, projectName, timelineName }: LayersControlsProps) {
  const layers = deriveLayerItemsFromTimeline(timeline)
    ...
    <>
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          Project: {projectName ?? "No project"} · Timeline: {timelineName ?? "No active timeline"}
        </p>
      </div>
          ...
          <div key={layer.id} className="space-y-2 rounded-md border border-border p-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <Button
                ...
                onClick={() => {
                  if (!timeline) return
                  onTimelineChange(toggleLayerVisibility(timeline, layer.id))
                }}
              >
                Toggle
              </Button>
              <Button
                ...
                onClick={() => {
                  if (!timeline) return
                  onTimelineChange(moveLayer(timeline, layer.id, "up"))
                }}
              >
                Move up
              </Button>
              <Button
                size="sm"
                variant="outline"
                disabled={!timeline || index === layers.length - 1}
                onClick={() => {
                  if (!timeline) return
                  onTimelineChange(moveLayer(timeline, layer.id, "down"))
                }}
              >
                Move down
              </Button>
