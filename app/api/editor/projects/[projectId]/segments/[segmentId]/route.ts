...
export async function GET(request: Request, context: { params: Promise<{ projectId: string; segmentId: string }> }) {
...
export async function PATCH(request: Request, context: { params: Promise<{ projectId: string; segmentId: string }> }) {
  const params = await context.params
  const auth = await requireEditorOperation(request, "edit_timeline")
  if ("error" in auth) return auth.error
  const { projectId, segmentId } = params
