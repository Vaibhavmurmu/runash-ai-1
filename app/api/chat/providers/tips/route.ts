...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) {
    return Response.json({ error: "Invalid tip provider request" }, { status: 400 })
  }

  const response: RecommendationProviderResponse<SustainabilityTip> = await chatRecommendationProviderService.getTips(parsed.data)
