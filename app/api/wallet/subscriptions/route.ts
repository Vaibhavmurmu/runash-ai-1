...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ success: true, data: await WalletStore.listSubscriptions(userId) })
}
...
export async function PATCH(request: NextRequest) {
  ...
  const updated = await WalletStore.updateSubscription(body.userId || "", body.subscriptionId, body.status)
  if (!updated) {
